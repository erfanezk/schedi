...
describe('OneTimeTaskRunner', () => {
	...
	describe('Task Execution', () => {
		...
		it('should not execute task after its expiration', () => {
			...
			// when
			taskRunner.start()
			vi.advanceTimersByTime(2000) // Task should be executed
			vi.advanceTimersByTime(2000) // Task should be expired and not executed again
		...
		it('should execute task immediately if startAt is in the past', () => {
			...
			// when
			taskRunner.start()
			vi.advanceTimersByTime(10) // Allow the task to run immediately since it's in the past
	...
	describe('Task Removal and Updates', () => {
		...
		it('should update a task correctly', () => {
			...
			// when
			taskRunner.start()
			vi.advanceTimersByTime(1000) // Let the task start

			const updatedTask = taskRunner.updateTask(task.id, {
